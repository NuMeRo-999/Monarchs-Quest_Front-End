...
const Enemies = ({ enemies, setSelectedEnemy, selectedEnemy, isAttacking }) => {

  return (
    <>
      <div
        className={`size-[192px] bg-no-repeat  z-10 absolute hover:drop-shadow-[0_50px_50px_rgba(255,255,255,1)] image-cursor right-[10%] top-[20%] ${enemies[0]?.state === 0 ? 'enemy-die bg-left-bottom' : 'enemy-idle bg-right-top'} ${isAttacking && enemies[0]?.id === selectedEnemy ? 'enemy-hit' : ''}`}
        id={enemies[0]?.id}
        style={{
      ...
      <div
        className={`size-[192px] bg-no-repeat  z-10 absolute hover:drop-shadow-[0_35px_35px_rgba(255,255,255,0.25)] image-cursor right-[20%] top-[30%] ${enemies[1]?.state === 0 ? 'enemy-die bg-left-bottom' : 'enemy-idle bg-right-top'} ${isAttacking && enemies[1]?.id === selectedEnemy ? 'enemy-hit' : ''}`}
        id={enemies[1]?.id}
        style={{
      ...
      <div
        className={`size-[192px] bg-no-repeat z-10 absolute hover:drop-shadow-[0_35px_35px_rgba(255,255,255,0.25)] image-cursor right-[10%] top-[40%] ${enemies[2]?.state === 0 ? 'enemy-die bg-left-bottom' : 'enemy-idle bg-right-top'} ${isAttacking && enemies[2]?.id === selectedEnemy ? 'enemy-hit' : ''}`}
        id={enemies[2]?.id}
        style={{
